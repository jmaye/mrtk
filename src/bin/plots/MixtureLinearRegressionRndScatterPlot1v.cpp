...
int main(int argc, char** argv) {
  ...
  distributions.push_back(LinearRegression<2>(LinearBasisFunction<double, 2>(
    Eigen::Matrix<double, 2, 1>(1.0, 1.0)), 1.0));
  distributions.push_back(LinearRegression<2>(LinearBasisFunction<double, 2>(
    Eigen::Matrix<double, 2, 1>(2.0, 2.0)), 1.0));
  distributions.push_back(LinearRegression<2>(LinearBasisFunction<double, 2>(
    Eigen::Matrix<double, 2, 1>(-1.0, -1.0)), 1.0));
  distributions.push_back(LinearRegression<2>(LinearBasisFunction<double, 2>(
    Eigen::Matrix<double, 2, 1>(-2.0, -2.0)), 1.0));
  ...
  for (size_t i = 0; i < 100; ++i)
    for (double x = -10; x < 10; x += 0.01) {
      for (size_t j = 0; j < distributions.size(); ++j)
        distributions[j].setBasis((Eigen::Matrix<double, 1, 1>() << x).
          finished());
